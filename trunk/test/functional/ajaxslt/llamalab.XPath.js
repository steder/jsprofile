  ...
  nodeName : function (l) {
    ...
    if (this.m[2]) {
      this.js.splice(l,0,'function(n){if(!nsr)throw new XPathException(14);return "',
        this.m[2],'"==((n.localName||n.baseName)&&nsr.lookupNamespaceURI("',
...
XPath.localName = function (nl) {
  return (nl.length && (nl[0].localName || nl[0].baseName)) || '';
};
...
XPath.sum = function (nl) {
  var r = 0, i = nl.length;
  while (--i >= 0) r += XPath.number(nl[i]);
  return r;
};
// Runtime - Axis
XPath.walk = function (nl, n) {
...
XPath.axes = {
  ...
  'attribute' : function (n) {
    return n.attributes ? Array.prototype.filter.call(n.attributes, this.specified) : [];
  },
  'child' : function (n) {
    return n.childNodes || [];
  },
 ...
 * @returns number 0 or with some of the above bits set.
 */
/**
